...
def _print_budget_estimate(estimate, usage_summary, max_total_tokens):
    ...
    estimated_total = estimate["estimated_total_tokens"]

    if max_total_tokens is None:
        print(Fore.WHITE + f"  [budget] {stage}: estimated {_format_tokens(estimated_total)} tokens.")
        return

    remaining = max_total_tokens - usage_summary["total_tokens"]
    projected = usage_summary["total_tokens"] + estimated_total
    print(
        Fore.WHITE
        + f"  [budget] {stage}: estimated {_format_tokens(estimated_total)} tokens; "
        + f"remaining {_format_tokens(remaining)}; projected {_format_tokens(projected)}/{_format_tokens(max_total_tokens)}."
    )


def _estimated_budget_available(usage_summary, estimate, max_total_tokens):
    if max_total_tokens is None:
        return True
...
def run_council(
    ...
    while attempt < max_retries:
        ...
        preflight_estimate = estimate_attempt_budget(
            user_prompt,
            generation_models,
            evaluation_personas,
            evaluation_models,
        ...
        _record_budget_estimate(usage_summary, preflight_estimate, attempt, status="preflight")
        ...
        if not _estimated_budget_available(usage_summary, generation_estimate, max_total_tokens):
            return None
        ...
        if not _estimated_budget_available(usage_summary, evaluation_estimate, max_total_tokens):
            return None
        ...
        semantic_estimate = _record_budget_estimate(
            usage_summary,
            estimate_semantic_entropy_stage(user_prompt, anonymised, SEMANTIC_ENTROPY_MODEL),
            attempt,
        )
        _print_budget_estimate(semantic_estimate, usage_summary, max_total_tokens)
        if not _estimated_budget_available(usage_summary, semantic_estimate, max_total_tokens):
            return None

        semantic_entropy, semantic_usage = analyse_semantic_entropy(user_prompt, anonymised, response_scores)
        _complete_budget_estimate(semantic_estimate, semantic_usage)
